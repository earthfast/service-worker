...
export class Driver implements Debuggable, UpdateSource {
   ...
   */
  private _state: DriverReadyState = DriverReadyState.NORMAL;
  public get state(): DriverReadyState {
    return this._state;
  }
  public set state(value: DriverReadyState) {
    this._state = value;
  }
   ...
   */
  protected clientVersionMap = new Map<ClientId, ManifestHash>();

  /**
   * Maps manifest hashes to instances of `AppVersion` for those manifests.
   */
  protected versions = new Map<ManifestHash, AppVersion>();
   ...
   */
  protected latestHash: ManifestHash|null = null;

  protected lastUpdateCheck: number|null = null;

  /**
   * Whether there is a check for updates currently scheduled due to navigation.
   */
  protected scheduledNavUpdateCheck: boolean = false;
  ...
  protected ngswStatePath = this.adapter.parseUrl('ngsw/state', this.scope.registration.scope).path;
  ...
  protected controlTable = this.db.open('control');
  ...
  protected async ensureInitialized(event: ExtendableEvent): Promise<void> {
  ...
  protected async handleMessage(msg: MsgAny&{action: string}, from: Client): Promise<void> {
  ...
  protected async handleFetch(event: FetchEvent): Promise<Response> {
    try {
   ...
   */
  protected async assignVersion(event: FetchEvent): Promise<AppVersion|null> {
   ...
   */
  protected async fetchLatestManifest(ignoreOfflineError?: false): Promise<Manifest>;
  protected async fetchLatestManifest(ignoreOfflineError: true): Promise<Manifest|null>;
  protected async fetchLatestManifest(ignoreOfflineError = false): Promise<Manifest|null> {
    const res =
  ...
  protected async deleteAllCaches(): Promise<void> {
   ...
   */
  protected async scheduleInitialization(appVersion: AppVersion): Promise<void> {
  ...
  protected async versionFailed(appVersion: AppVersion, err: Error): Promise<void> {
  ...
  protected async setupUpdate(manifest: Manifest, hash: string): Promise<void> {
    try {
  ...
  // Armada: this was added for the explicit purpose of being overriden.
  protected newAppVersion(manifest: Manifest, hash: ManifestHash): AppVersion {
