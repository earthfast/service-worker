...
import {normalizeUrl, parseUrlArmada as parseUrl} from '../utils';
...
export class SwTestHarnessBuilder {
  ...
  build(): SwTestHarness {
    const harness = new SwTestHarnessImpl(this.server, this.caches, this.scopeUrl) as SwTestHarness;
...
export class SwTestHarnessImpl extends Adapter<MockCacheStorage> implements
  ...
  fetch(req: RequestInfo): Promise<Response> {
    let urlStr = (typeof req === 'string') ? req : (req as Request).url;
    let url = new URL(normalizeUrl(urlStr, this.scopeUrl), this.scopeUrl);

    // Remove the cache bust parameter since it's random, even in tests.
    url.searchParams.delete(ArmadaAPIClientImpl.cacheBustKey);

    let mockReq = new MockRequest(normalizeUrl(url.toString(), this.scopeUrl));
  ...
  override newResponse(body: any, init: Object = {}): Response {
